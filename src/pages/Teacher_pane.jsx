function Teacher_pane() {
    return (
        <>
            <div className="head">
                <div className='container'>
                    <div className="icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 16 16"><g fill="#0a81ff"><path d="M11 6a3 3 0 1 1-6 0a3 3 0 0 1 6 0" /><path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1" /></g></svg>

                    </div>
                    <div className='Tech_Panel'>
                        <h5 className='Techer'>Akbarov Abdulaziz</h5>
                        <h5 className='Soxa'>Frontend react
                            <svg className='Soxa_icon'
                                xmlns="http://www.w3.org/2000/svg"
                                width="1.13em"
                                height="1em"
                                viewBox="0 0 256 228">
                                <path fill="#00d8ff"
                                    d="M210.483 73.824a171.49 171.49 0 0 0-8.24-2.597c.465-1.9.893-3.777 1.273-5.621c6.238-30.281 2.16-54.676-11.769-62.708c-13.355-7.7-35.196.329-57.254 19.526a171.23 171.23 0 0 0-6.375 5.848a155.866 155.866 0 0 0-4.241-3.917C100.759 3.829 77.587-4.822 63.673 3.233C50.33 10.957 46.379 33.89 51.995 62.588a170.974 170.974 0 0 0 1.892 8.48c-3.28.932-6.445 1.924-9.474 2.98C17.309 83.498 0 98.307 0 113.668c0 15.865 18.582 31.778 46.812 41.427a145.52 145.52 0 0 0 6.921 2.165a167.467 167.467 0 0 0-2.01 9.138c-5.354 28.2-1.173 50.591 12.134 58.266c13.744 7.926 36.812-.22 59.273-19.855a145.567 145.567 0 0 0 5.342-4.923a168.064 168.064 0 0 0 6.92 6.314c21.758 18.722 43.246 26.282 56.54 18.586c13.731-7.949 18.194-32.003 12.4-61.268a145.016 145.016 0 0 0-1.535-6.842c1.62-.48 3.21-.974 4.76-1.488c29.348-9.723 48.443-25.443 48.443-41.52c0-15.417-17.868-30.326-45.517-39.844m-6.365 70.984c-1.4.463-2.836.91-4.3 1.345c-3.24-10.257-7.612-21.163-12.963-32.432c5.106-11 9.31-21.767 12.459-31.957c2.619.758 5.16 1.557 7.61 2.4c23.69 8.156 38.14 20.213 38.14 29.504c0 9.896-15.606 22.743-40.946 31.14m-10.514 20.834c2.562 12.94 2.927 24.64 1.23 33.787c-1.524 8.219-4.59 13.698-8.382 15.893c-8.067 4.67-25.32-1.4-43.927-17.412a156.726 156.726 0 0 1-6.437-5.87c7.214-7.889 14.423-17.06 21.459-27.246c12.376-1.098 24.068-2.894 34.671-5.345a134.17 134.17 0 0 1 1.386 6.193M87.276 214.515c-7.882 2.783-14.16 2.863-17.955.675c-8.075-4.657-11.432-22.636-6.853-46.752a156.923 156.923 0 0 1 1.869-8.499c10.486 2.32 22.093 3.988 34.498 4.994c7.084 9.967 14.501 19.128 21.976 27.15a134.668 134.668 0 0 1-4.877 4.492c-9.933 8.682-19.886 14.842-28.658 17.94M50.35 144.747c-12.483-4.267-22.792-9.812-29.858-15.863c-6.35-5.437-9.555-10.836-9.555-15.216c0-9.322 13.897-21.212 37.076-29.293c2.813-.98 5.757-1.905 8.812-2.773c3.204 10.42 7.406 21.315 12.477 32.332c-5.137 11.18-9.399 22.249-12.634 32.792a134.718 134.718 0 0 1-6.318-1.979m12.378-84.26c-4.811-24.587-1.616-43.134 6.425-47.789c8.564-4.958 27.502 2.111 47.463 19.835a144.318 144.318 0 0 1 3.841 3.545c-7.438 7.987-14.787 17.08-21.808 26.988c-12.04 1.116-23.565 2.908-34.161 5.309a160.342 160.342 0 0 1-1.76-7.887m110.427 27.268a347.8 347.8 0 0 0-7.785-12.803c8.168 1.033 15.994 2.404 23.343 4.08c-2.206 7.072-4.956 14.465-8.193 22.045a381.151 381.151 0 0 0-7.365-13.322m-45.032-43.861c5.044 5.465 10.096 11.566 15.065 18.186a322.04 322.04 0 0 0-30.257-.006c4.974-6.559 10.069-12.652 15.192-18.18M82.802 87.83a323.167 323.167 0 0 0-7.227 13.238c-3.184-7.553-5.909-14.98-8.134-22.152c7.304-1.634 15.093-2.97 23.209-3.984a321.524 321.524 0 0 0-7.848 12.897zm8.081 65.352c-8.385-.936-16.291-2.203-23.593-3.793c2.26-7.3 5.045-14.885 8.298-22.6a321.187 321.187 0 0 0 7.257 13.246c2.594 4.48 5.28 8.868 8.038 13.147m37.542 31.03c-5.184-5.592-10.354-11.779-15.403-18.433c4.902.192 9.899.29 14.978.29c5.218 0 10.376-.117 15.453-.343c-4.985 6.774-10.018 12.97-15.028 18.486m52.198-57.817c3.422 7.8 6.306 15.345 8.596 22.52c-7.422 1.694-15.436 3.058-23.88 4.071a382.417 382.417 0 0 0 7.859-13.026a347.403 347.403 0 0 0 7.425-13.565m-16.898 8.101a358.557 358.557 0 0 1-12.281 19.815a329.4 329.4 0 0 1-23.444.823c-7.967 0-15.716-.248-23.178-.732a310.202 310.202 0 0 1-12.513-19.846h.001a307.41 307.41 0 0 1-10.923-20.627a310.278 310.278 0 0 1 10.89-20.637l-.001.001a307.318 307.318 0 0 1 12.413-19.761c7.613-.576 15.42-.876 23.31-.876H128c7.926 0 15.743.303 23.354.883a329.357 329.357 0 0 1 12.335 19.695a358.489 358.489 0 0 1 11.036 20.54a329.472 329.472 0 0 1-11 20.722m22.56-122.124c8.572 4.944 11.906 24.881 6.52 51.026c-.344 1.668-.73 3.367-1.15 5.09c-10.622-2.452-22.155-4.275-34.23-5.408c-7.034-10.017-14.323-19.124-21.64-27.008a160.789 160.789 0 0 1 5.888-5.4c18.9-16.447 36.564-22.941 44.612-18.3M128 90.808c12.625 0 22.86 10.235 22.86 22.86s-10.235 22.86-22.86 22.86s-22.86-10.235-22.86-22.86s10.235-22.86 22.86-22.86" /></svg>
                        </h5>
                    </div>
                    <div className="exit">
                        <button className='btn_exit'>
                            <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="1em" viewBox="0 0 15 15"><path fill="none" stroke="#0a81ff" d="m13.5 7.5l-3 3.25m3-3.25l-3-3m3 3H4m4 6H1.5v-12H8" /></svg>
                        </button>
                    </div>
                </div>
                <div className="Container_grup">
                    <h3 className="grup">Guruhlar</h3>
                </div>
                <div className="Container_card">
                    <div className="Group">
                    <a href="#Frontend react js" className="Container_group_name">
                        Frontend react js
                    </a>
                    </div>
                    <div className="Container_id">
                        <svg xmlns="http://www.w3.org/2000/svg" width="2rem" height="2rem" viewBox="0 0 512 512"><path fill="#80d4ff" d="M50 96c-9.972 0-18 8.028-18 18v300c0 9.972 8.028 18 18 18h412c9.972 0 18-8.028 18-18V114c0-9.972-8.028-18-18-18zm5 23h402v18H55zm0 32h210v258H55zm18 18v222h7.03c.47-24.342 18.315-74.172 47.093-97.889C136.331 300.56 147.615 305 160 305c12.385 0 23.669-4.44 32.877-11.889C221.655 316.828 239.5 366.658 239.97 391H247V169zm222 14h146v18H295zm-135 10c20.835 0 39 20.241 39 47s-18.165 47-39 47s-39-20.241-39-47s18.165-47 39-47m135 22h114v18H295zm0 32h50v18h-50zm80 0h66v18h-66zm-16 96h98v18h-98zm-32 32h130v18H327z" /></svg>
                        <div className="ID">
                        <h3 className="Fn">FN11</h3>
                        </div>
                        <div className="people_number">
                    <svg xmlns="http://www.w3.org/2000/svg" width="2rem" height="2rem" viewBox="0 0 24 24"><path fill="#ff8080" d="M12 12.75c1.63 0 3.07.39 4.24.9c1.08.48 1.76 1.56 1.76 2.73V18H6v-1.61c0-1.18.68-2.26 1.76-2.73c1.17-.52 2.61-.91 4.24-.91M4 13c1.1 0 2-.9 2-2s-.9-2-2-2s-2 .9-2 2s.9 2 2 2m1.13 1.1c-.37-.06-.74-.1-1.13-.1c-.99 0-1.93.21-2.78.58A2.01 2.01 0 0 0 0 16.43V18h4.5v-1.61c0-.83.23-1.61.63-2.29M20 13c1.1 0 2-.9 2-2s-.9-2-2-2s-2 .9-2 2s.9 2 2 2m4 3.43c0-.81-.48-1.53-1.22-1.85A6.95 6.95 0 0 0 20 14c-.39 0-.76.04-1.13.1c.4.68.63 1.46.63 2.29V18H24zM12 6c1.66 0 3 1.34 3 3s-1.34 3-3 3s-3-1.34-3-3s1.34-3 3-3"/></svg>
                    <div className="number_people">
                    <h3 className="number">22 ta</h3>
                    </div>
                    </div>
                    </div>
                </div>
                {/* <div className="main">
                <div className="form">
                <h6 className="id">ID</h6>
                <svg className="bottom" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 32 32"><path fill="none" stroke="#0a81ff" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="m6 22l10 8l10-8m-10 8V2"/></svg>
                <h6 className="grop">Guruhlar</h6>
                <h6 className="children">O'quvchilar soni </h6>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd">1</h4>
                    <a href="#Frontend react js" className="grup">Frontend react js</a>
                    <h4 className="people">22 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd">2</h4>
                    <a href="#Flutter Mobile st" className="grup">Flutter Mobile st</a>
                    <h4 className="people">12 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd_3">3</h4>
                    <a href="#Backend standart" className="grup_3">Backend standart</a>
                    <h4 className="people_3">24 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd_4">4</h4>
                    <a href="#SMM Maneger st" className="grup_4">SMM Maneger st</a>
                    <h4 className="people_4">18 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd">5</h4>
                    <a href="#Paython standart" className="grup">Paython standart</a>
                    <h4 className="people">15 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd_6">6</h4>
                    <a href="#Frontend React js" className="grup_6">Frontend React js</a>
                    <h4 className="people_6">19 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd">7</h4>
                    <a href="#Frontend Node js" className="grup">Frontend Node js</a>
                    <h4 className="people">16 ta</h4>
                 </div>

                 <div className="form_panel">
                    <h4 className="idd">8</h4>
                    <a href="#Frontend Next js" className="grup">Frontend Next js</a>
                    <h4 className="people">20 ta</h4>
                 </div>
            </div>*/ }
            </div>
        </>
    )
}

export default Teacher_pane;